...
def train(hyp):
    ...
    # Model parameters
    model.nc = nc  # attach number of classes to model   将类别绑定到模型中
    model.hyp = hyp  # attach hyperparameters to model   将参数绑定到这个模型中
    ...
    for epoch in range(opt.start_epoch, epochs):
        mloss, lr = train_util.train_one_epoch(model, optimizer, train_dataloader,
                                               ...
                                               scaler=scaler)
        # update scheduler
        train_loss.append(mloss[3].item())
        ...
        if opt.notest is False or epoch == epochs - 1:
            ...
            val_map.append(voc_mAP)  # pascal mAP
            ...
            if opt.savebest is False:
                ...
                with open(results_file, 'r') as f:
                    ...
                    torch.save(save_files, "./weights/yolov3spp01-{}.pt".format(epoch))
            else:
