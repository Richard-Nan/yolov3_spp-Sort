...
def compute_loss(p, targets, model):  # predictions, targets, model
    device = p[0].device
    lcls = torch.zeros(1, device=device)  # Tensor(0) 分類損失
    lbox = torch.zeros(1, device=device)  # Tensor(0) 定位损失
    lobj = torch.zeros(1, device=device)  # Tensor(0) obj损失
...
def build_targets(p, targets, model):
    ...
    for i, j in enumerate(model.yolo_layers):  # j: [89, 101, 113]
        ...
        # Match targets to anchors   targets * gain 特征层绝对坐标
        ...
        if nt:  # 如果存在target的话
            ...
            # 获取正样本对应的anchor模板与target信息 t 就是正样本的个数
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6,
    ...
    t = time.time()
    # prediction   # view [1, 3, 13, 13, 85] as [1, 507, 85]
